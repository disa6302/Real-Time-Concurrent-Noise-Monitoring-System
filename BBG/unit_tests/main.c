...
enum Status api_send_socket_log(logpacket msg)
{
    ...
    status=mq_send(log_queue, (const char*)&msg, sizeof(msg),1);
    if(status == -1)
    {
...
enum Status api_send_socket_dec(logpacket msg)
{
    ...
    status=mq_send(decision_queue, (const char*)&msg, sizeof(msg),1);
    if(status == -1)
    {
        printf("\nERR: Socket Task was unable to send log message\n");
        return FAIL;
...
void *app_sync_logger(void *args) // SocketThread/Task
{
    ...
    while(1)
    {
        ...
        logpacket temp; 
        temp.req_type = -1;
        status = mq_receive(log_queue,(char*)&temp, sizeof(temp), NULL);
        if(temp.logmsg !=NULL)
        {
            if(status >0)
            {
...
void *app_decision_task(void *args) // SocketThread/Task
{
    ...
    while(1)
    {
        ...
        temp.req_type = -1;
        status = mq_receive(decision_queue,(char*)&temp, sizeof(temp), NULL);
        printf("\nLog Status %d\n",status);
        if(temp.logmsg !=NULL)
        {
            if(status >0)
